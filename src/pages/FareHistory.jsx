...
export const FareHistory = () => {
  ...
  // Deterministic demo series (stable between renders; differs per route)
  const fareData = useMemo(() => {
    const rand = seededRandom(hashString(route));
    return Array.from({ length: 60 }, (_, i) => {
      const base = route === 'DEL-BOM' ? 5200 : route === 'BLR-DEL' ? 6100 : 4200;
      const noise = rand() * 2000 - 1000;
        ...
        <div className="h-[280px] mt-3">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={fareData}>
              <defs>
                {[['indigoGrad','#1F4FBF'],['aiGrad','#ef4444'],['vistaraGrad','#8b5cf6']].map(([id,color]) => (
              ...
              <YAxis domain={[2500,12000]} stroke="#66748F" tick={{ fontSize:10, fill:'#66748F' }} tickLine={false} tickFormatter={v=>`₹${v}`} />
              <Tooltip content={<LightTooltip />} />
              <ReferenceLine x="Day 41" stroke="#F58A1F" strokeDasharray="3 3" label={{ value:'Surge', fill:'#F58A1F', fontSize:9 }} />
              <Area type="monotone" dataKey="indigo"   name="IndiGo"    stroke="#1F4FBF" strokeWidth={2} fill="url(#indigoGrad)" />
