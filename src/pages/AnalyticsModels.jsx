...
export const AnalyticsModels = () => {
  ...
  const anomalyData = useMemo(() => {
    ...
    return Array.from({ length: 60 }, (_, i) => {
      const base = 5400;
      const noise = rand() * 800 - 400;
      const surge = [21, 39, 52].includes(i) ? rand() * 3500 + 2000 : 0;
            ...
            <div className="h-[280px] mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={anomalyData}>
                  ...
                  <XAxis dataKey="day" stroke="#66748F" tick={{ fontSize:9, fill:'#66748F' }} tickLine={false} interval={9} />
                  <YAxis domain={[2000,12000]} stroke="#66748F" tick={{ fontSize:10, fill:'#66748F' }} tickLine={false} tickFormatter={v=>`₹${v}`} />
                  ...
                  <Line type="monotone" dataKey="mean7"  name="7d μ" stroke="#66748F" strokeWidth={1.5} dot={false} />
                  <Line type="monotone" dataKey="fare"   name="Fare" stroke="#1F4FBF" strokeWidth={2}   dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-[300px] mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={forecastData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#DFE5EF" vertical={false} />
                  <XAxis dataKey="month" stroke="#66748F" tick={{ fontSize:10, fill:'#66748F' }} tickLine={false} />
                  <YAxis domain={[4000,9000]} stroke="#66748F" tick={{ fontSize:10, fill:'#66748F' }} tickLine={false} tickFormatter={v=>`₹${v}`} />
                  <Tooltip content={<LightTooltip />} />
                  <ReferenceLine x="Nov-24" stroke="#66748F" strokeDasharray="3 3" label={{ value:'Forecast →', fill:'#66748F', fontSize:9 }} />
                  <Line type="monotone" dataKey="actual" name="Actual" stroke="#1F4FBF" strokeWidth={2.5} dot={{ r:3, fill:'#1F4FBF' }} connectNulls={false} />
            ...
            <div className="h-[300px] mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={decayData}>
                  <defs>
                    {[['iGrad','#1F4FBF'],['aGrad','#ef4444'],['vGrad','#7c3aed']].map(([id,c]) => (
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="#DFE5EF" vertical={false} />
                  <XAxis dataKey="days" reversed stroke="#66748F" tick={{ fontSize:10, fill:'#66748F' }} tickLine={false} tickFormatter={v=>`D-${v}`} />
                  <YAxis domain={[3000,18000]} stroke="#66748F" tick={{ fontSize:10, fill:'#66748F' }} tickLine={false} tickFormatter={v=>`₹${v}`} />
                  <Tooltip content={<LightTooltip />} />
                  <ReferenceLine x={14} stroke="#F58A1F" strokeDasharray="3 3" label={{ value:'D-14 Crossover', fill:'#F58A1F', fontSize:9 }} />
                  <Area type="monotone" dataKey="indigo"  name="IndiGo"    stroke="#1F4FBF" strokeWidth={2} fill="url(#iGrad)" />
