    ...
    <aside className="w-60 bg-slate-900 flex flex-col justify-between shrink-0 h-full z-40 select-none">
      <div>
        <div className="px-5 py-4 border-b border-white/10 flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-[15px] font-bold tracking-tight text-white leading-tight">AFPI</h1>
          ...
          {navItems.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`relative w-full flex items-center space-x-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${active ? 'bg-white/10 text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
              >
                {active && <span className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-r bg-saffron-500"></span>}
      ...
      <div className="p-3 border-t border-white/10 space-y-2.5">
        ...
        <button onClick={onOpenProfile} className="w-full flex items-center space-x-2.5 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors text-left group">
          <div className="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold shrink-0 ring-2 ring-white/10 group-hover:ring-saffron-400 transition-all">HJ</div>
